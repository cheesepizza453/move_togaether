...
const ShelterMapPage = () => {
  ...
  const fetchPostsData = async () => {
    try {
      ...
      if (result.success) {
        console.log('[shelter] 전체 게시물 수:', result.data.length);
        console.log('[shelter] 게시물 departure 샘플:', result.data.slice(0, 3).map(p => ({
          id: p.id,
          address: p.departure?.address,
          lat: p.departure?.lat,
          lng: p.departure?.lng,
          dong: p.departure?.dong,
        })));
  ...
  const updateMarkers = useCallback(() => {
    ...
    console.log('[shelter] 좌표 없는 게시물(주소 지오코딩 대상):', addressOnlyPosts.length);
    console.log('[shelter] Geocoder 사용 가능:', !!window.kakao?.maps?.services?.Geocoder);

    if (addressOnlyPosts.length > 0 && window.kakao?.maps?.services?.Geocoder) {
      const geocoder = new window.kakao.maps.services.Geocoder();

      // 주소 문자열별로 그룹핑 (같은 주소는 한 번만 지오코딩)
      const groupedByAddress = addressOnlyPosts.reduce((acc, post) => {
        const addr = post.departure.address;
        if (!acc[addr]) acc[addr] = [];
        acc[addr].push(post);
        return acc;
      }, {});

      console.log('[shelter] 지오코딩할 주소 목록:', Object.keys(groupedByAddress));

      Object.entries(groupedByAddress).forEach(([address, posts]) => {
        geocoder.addressSearch(address, (result, status) => {
          console.log(`[shelter] Geocoder 결과 - 주소: "${address}", status: ${status}, 결과:`, result?.[0]);
          if (status !== window.kakao.maps.services.Status.OK || !result[0]) return;

          const lat = parseFloat(result[0].y);
          const lng = parseFloat(result[0].x);
          console.log(`[shelter] 마커 생성 - 주소: "${address}", lat: ${lat}, lng: ${lng}`);

          const postsWithCoords = posts.map(post => ({
            ...post,
            departure: { ...post.departure, lat, lng }
          }));

          createAndAddMarkers(postsWithCoords);
        });
      });
    }
  ...
  // 데이터가 도착했을 때 지도가 이미 초기화되어 있으면 마커 업데이트
  useEffect(() => {
    if (mapInstanceRef.current && clustererRef.current && postsData.length > 0) {
      updateMarkers();
    }
  ...
  const loadKakaoMapScript = (retryCount = 0) => {
    if (!document.querySelector('script[src*="dapi.kakao.com"]')) {
      ...
      script.src = `https://dapi.kakao.com/v2/maps/sdk.js?appkey=${process.env.NEXT_PUBLIC_KAKAO_MAPS_API_KEY}&libraries=clusterer,services&autoload=false`;
      script.async = false;
